...
def parse_args(args):
    ...
    parser.add_argument('--max-detections', help='Max Detections per image (defaults to 100).', default=1000000, type=int)
...
def main(args=None):
    ...
    if args.force_hard_score:
        force_hard_score = args.force_hard_score.lower() == "true"
    else:
        force_hard_score = False
    print ("force_hard_score={}".format(force_hard_score))
    ...
    # start evaluation
    if args.dataset_type == 'coco':
        from object_detector_retinanet.keras_retinanet.utils.coco_eval import evaluate_coco
