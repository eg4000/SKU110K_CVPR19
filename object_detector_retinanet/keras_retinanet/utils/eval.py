...
def _get_detections(generator, model, score_threshold=0.05, max_detections=300, save_path=None, save_detections=False):
    ...
    csv_data_lst = []
    csv_data_lst.append(['image_id', 'x1', 'y1', 'x2', 'y2', 'confidence', 'label'])
    for i in range(generator.size()):
        ...
        if save_detections:
            for box, score, label in zip(image_boxes, image_scores, image_labels):
                ...
                # row = [os.path.basename(generator.image_path(i)).split('.')[0], x1, y1, x2, y2, score, label]
                row = [image_name, x1, y1, x2, y2, score, label]
